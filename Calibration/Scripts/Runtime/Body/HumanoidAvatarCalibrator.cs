namespace Games.NoSoySauce.Avatars.Calibration.Body
{
    ...
    /// <summary>
    ///     Allows to apply <see cref="BodyCalibrationProfile" /> to a humanoid avatar.
    /// </summary>
    public class HumanoidAvatarCalibrator : MonoBehaviour
    {
        ...
        public static void ApplyCalibrationProfileToAvatar(BodyCalibrationProfile profile, Transform avatarRoot)
        {
            ...
            // Get visor transform.
            var avatarVisorTag = avatarAnimator.GetComponentInChildren<CenterEyeTag>(); //avatarBones.head.Find("visor");
            if (avatarVisorTag == null)
            {
                Debug.LogError($"Cannot calibrate avatar '{avatarRoot.name}': avatar needs to have a transform with a '{nameof(CenterEyeTag)}' component attached. It represents avatar's sight origin. Visor's Z axis should point in the direction of sight, Y axis - upwards. Please add it. Calibration will not work without it.");
                return;
            }
